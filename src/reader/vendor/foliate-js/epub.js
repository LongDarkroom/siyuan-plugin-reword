...
// [REword] 2026-08-24：某些 EPUB 对字体做混淆后会使用 .eno1 / .odt / .dat 等非标准扩展名，
// 导致 foliate-js 无法识别为字体而直接把原始 href 写回 CSS，浏览器请求相对路径产生 404。
const isFontPath = (path) => {
    const ext = path.toLowerCase().split('.').pop()
    if (!ext) return false
    return FONT_EXTENSIONS.includes(ext)
        || ext.startsWith('eno') || ext.startsWith('odt') || ext === 'dat' || ext === 'bin'
}
...
class Loader {
    ...
    async loadHref(href, base, parents = []) {
        ...
        if (!item) {
            ...
            if (!item) {
                // [REword] 2026-08-24：缺失的内部资源不要保留原始 href，否则 iframe 会请求相对路径
                // （如 plugin:siyuan-plugin-reword/kk.eno1）并报 404。返回空字符串让浏览器静默失败。
                return ''
            }
        }
