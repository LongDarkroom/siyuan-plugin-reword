...
// [REword] 2026-08-23 问题1修复：EPUB XML（nav/ncx/opf）编码探测解码
...
const makeZipLoader = async file => {
    // [REword] 2026-08-23 问题1修复：EPUB 的 nav/ncx/OPF 可能是 GBK 编码，
    // 官方 TextWriter 固定 UTF-8 会解出乱码目录 → 改用 blob + decodeText 自动探测。
...
const makeDirectoryLoader = async entry => {
    ...
    // [REword] 2026-08-23 问题1修复：同 makeZipLoader，目录模式也走探测解码
    const getBuffer = name => map.get(name)?.arrayBuffer() ?? null
    const loadText = async name => {
        const buf = await getBuffer(name)
        return buf ? decodeText(new Uint8Array(buf)) : null
    }
