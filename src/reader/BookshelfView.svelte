  ...
  <div class="bookshelf-body">
    {#if sidebarOpen}
      ...
      <aside class="shelf-sidebar">
        ...
        <div class="shelf-sb-block">
          <div class="shelf-sb-title">阅读状态</div>
        ...
        <div class="shelf-sb-block">
          <div class="shelf-sb-title">
            <span>我的分组</span>
            <button class="shelf-sb-add" title="管理分组" on:click={() => (groupDialogOpen = true)}>＋</button>
          </div>
        ...
        {#if facets.tags.length}
          <div class="shelf-sb-block">
            <div class="shelf-sb-title">标签</div>
        ...
        {#if facets.formats.length > 1}
          <div class="shelf-sb-block">
            <div class="shelf-sb-title">格式</div>
            <div class="shelf-sb-tags">
          ...
          {#each visible as book (book.id)}
            ...
            >
              <div class="book-cover-wrap">
                ...
                >{STATUS_LABEL[book.status ?? "unread"]}</button>
              </div>
              <div class="book-info">
                ...
                {#if book.rating}
                  <div class="book-stars" title="{book.rating} 星">
                    {"★".repeat(book.rating)}<span class="book-stars-off">{"★".repeat(5 - book.rating)}</span>
                  </div>
                {/if}
                {#if book.tags?.length}
                  <div class="book-tags">
                    ...
                    {#if book.tags.length > 2}
                      <span class="book-tag more">+{book.tags.length - 2}</span>
                    {/if}
                  </div>
                {/if}
                <div class="book-meta">
            ...
            >
              ...
              <div class="row-main">
                <div class="row-title-line">
                  ...
                  {#if book.favorite}<span class="row-fav">★</span>{/if}
                  {#if book.series}<span class="row-series">{book.series}</span>{/if}
                </div>
                <div class="row-sub">
              ...
              >{STATUS_LABEL[book.status ?? "unread"]}</button>
              <span class="row-cell row-rating" title="评分">
                {book.rating ? "★".repeat(book.rating) : "—"}
              </span>
              <span class="row-cell">{fmtPct(book.progress?.fraction)}</span>
              <span class="row-cell">{fmtTime(book.readingTimeMs) || "—"}</span>
              <span class="row-cell">{fmtDate(book.lastReadAt ?? book.addedAt)}</span>
...
<style>
  ...
  .row-actions .book-btn {
    flex: initial;
    padding: 3px 8px;
  }
  /* 窄面板（dock 侧栏）下隐藏次要列，避免挤压 */
  @media (max-width: 560px) {
    .row-cell,
    .row-series {
      display: none;
    }
    .row-actions .book-btn.danger {
      display: none;
    }
  }
